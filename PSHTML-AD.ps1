...
param (
	...
	[String]$CompanyLogo = "\\ad.corp.example.com\SYSVOL\ad.corp.example.com\Files\company_logo.png",
	...
	[String]$RightLogo = "\\ad.corp.example.com\SYSVOL\ad.corp.example.com\Files\company_logo.png",
	#Title of generated report

	[Parameter(ValueFromPipeline = $true, HelpMessage = "Enter desired title for report")]
	[String]$ReportTitle = "[Company] Active Directory Report",
	...
	[String]$ReportSavePath = "F:\Scripts\Reporting\logs\ADReport\",
...
#Email Report
$reportfile = Save-HTMLReport -ReportContent $FinalReport -ShowReport -ReportName $ReportName -ReportPath $ReportSavePath
...
$subject = "Interactive Weekly AD Report" + " " + $subjectdate
$time = (Get-Date).Adddays(-($DaysInactive))
$body = 
    "Attached is an interactive HTML report generated with PowerShell of an overview of the [Company] Active Directory enviornment.

    Tech Ops Reporting"

Send-MailMessage -To $emailTo -From $emailFrom -Subject $subject -Body $body -Attachments $reportfile -SmtpServer $smtpserver